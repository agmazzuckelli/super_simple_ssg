...
function getMdFilesFromDir(dir, draftDir) {
  ...
  const nonDraftMdFiles = fs
    ...
    .filter(
      (dirent) => !dirent.isDirectory() && dirent.name.endsWith(extension),
    )
...
function createArticles(template, buildDir, mdDetails) {
  for (const [mdPath, metaMap, mdHtml] of mdDetails) {
    ...
    const title =
      metaMap.get("title") ?? toTitleCase(fileName.replaceAll("_", " "));
    const withTitle = injectIntoTemplate(withContent, "{{ title }}", title);
    const pubDate = metaMap.get("published_date")
      ? parseDate(metaMap.get("published_date"))
      : "";
    const withDate = injectIntoTemplate(
      withTitle,
      "{{ published_date }}",
      pubDate,
    );
    const lmDate = metaMap.get("last_modified_date")
      ? parseDate(metaMap.get("last_modified_date"))
      : pubDate;
    const compiledHtml = injectIntoTemplate(
      withDate,
      "{{ last_modified_date }}",
      lmDate,
    );
    ...
    const fileOutputDir =
      path.dirname(pathInBuild) + "/" + fileName.replaceAll("_", "-") + "/";
    fs.mkdirSync(fileOutputDir);
    fs.writeFile(
      fileOutputDir + "index.html",
      compiledHtml,
      { flag: "w+" },
      (err) => {
        if (err) {
...
function addArticles(template, contentDir, mdDetails) {
  ...
  for (const [mdPath, metaMap, _mdHTML] of mdDetails) {
    ...
    if (articlesPerYear.has(pubYear)) {
      articlesPerYear
        .get(pubYear)
        .push({ pubDate: pubDate, article: articleBullet });
    } else {
      articlesPerYear.set(pubYear, [
        { pubDate: pubDate, article: articleBullet },
      ]);
    }
  }
  ...
  for (const [year, bullets] of sortedArticlesPerYear) {
    const bulletsByDate = bullets.sort(
      (d1, d2) => new Date(d2.pubDate) - new Date(d1.pubDate),
    ); // desc
    let articles = "";
...
function createHomepage(template, sourceDir, contentDir, buildDir, mdDetails) {
  ...
  fs.writeFile(
    sourcePathToBuildPath(sourceDir + "index.html", buildDir, "index.html"),
    compiledHtml,
    { flag: "w+" },
    (err) => {
      if (err) {
...
function copyToBuild(resourcePath, resourceName, buildDir) {
  fs.cpSync(
    resourcePath,
    sourcePathToBuildPath(resourcePath, buildDir, resourceName),
    { recursive: true },
  );
}
...
function generateStaticSite() {
  ...
  const contentMdFiles = getMdFilesFromDir(
    contentDir,
    contentDir + draftSuffix + "/",
  );
